...
function InputTextWithAttachment(props) {
  ...
  return (
    <div className='bg-gray-50 w-full flex items-center rounded-md pr-4 '>
      <textarea type="text" value={text} onChange={(e) => handleChange(e)} onKeyDown={(e) => e.key==='Enter'? submitHandler(e) : null } className={`w-full rounded-md border-none outline-none overflow-hidden p-2 px-4 duration-200 text-black  ${loading ? 'primary-bg' : 'bg-gray-50'}` } disabled={loading} placeholder={`${loading ? 'Loading...': 'Type a message'}`}/>
      <div className="">
