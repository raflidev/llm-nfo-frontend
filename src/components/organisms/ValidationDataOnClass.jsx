        ...
        <>
          <div className='space-y-2'>
            {termItem.map((cqItem, indexCQ) => {
              return (
                <div className="" key={indexCQ}>
                  <div className="border border-black rounded p-4">
                    <h1 className='font-semibold'>{cqItem[0]}</h1>
                    {
                      cqItem[1].map((item, index) => {
                        return (
                          <div key={index+"-"+item} className='flex space-x-3 items-center'>
                            {!saveItem[index] ?
                            <textarea className='flex w-full bg-transparent border border-white p-1 rounded mt-4' value={item} onChange={(e) => changeHandle(e, index)} />
                            :
